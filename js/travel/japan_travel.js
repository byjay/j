...
// 일본 여행 지역 정보 (후쿠오카, 오사카 최상단)
const japanRegions = {
    // --- ★★★ TIER 1: 최우선 지역 (후쿠오카, 오사카) ★★★ ---
    fukuoka: { id: 'fukuoka', name: '후쿠오카', nameJp: '福岡', icon: '🏯', color: 'red', unlockCondition: 'default', description: '규슈 최대 도시', scriptFile: 'js/travel/fukuoka.js', mission: '🎉 바로 여행 가능!' },
    osaka: { id: 'osaka', name: '오사카', nameJp: '大阪', icon: '🍜', color: 'orange', unlockCondition: 'default', description: '맛의 도시', scriptFile: 'js/travel/osaka.js', mission: '🎉 바로 여행 가능!' },

    // --- 잠금 지역 (아빠 외 접근 불가 - 공부해야 열림) ---
    nagoya: { id: 'nagoya', name: '나고야', nameJp: '名古屋', icon: '🏯', color: 'yellow', unlockCondition: 'study_required', description: '히츠마부시와 지브리', scriptFile: 'js/travel/nagoya.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    kobe: { id: 'kobe', name: '고베', nameJp: '神戸', icon: '🥩', color: 'red', unlockCondition: 'study_required', description: '고베규와 디저트', scriptFile: 'js/travel/kobe.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    nara: { id: 'nara', name: '나라', nameJp: '奈良', icon: '🦌', color: 'green', unlockCondition: 'study_required', description: '사슴공원과 대불', scriptFile: 'js/travel/nara.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    kyoto: { id: 'kyoto', name: '교토', nameJp: '京都', icon: '⛩️', color: 'purple', unlockCondition: 'study_required', description: '전통과 문화의 도시', scriptFile: 'js/travel/kyoto.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    yokohama: { id: 'yokohama', name: '요코하마', nameJp: '横浜', icon: '🎡', color: 'blue', unlockCondition: 'study_required', description: '항구도시와 야경', scriptFile: 'js/travel/yokohama.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    hakone: { id: 'hakone', name: '하코네', nameJp: '箱根', icon: '♨️', color: 'indigo', unlockCondition: 'study_required', description: '온천과 후지산', scriptFile: 'js/travel/hakone.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    sendai: { id: 'sendai', name: '센다이', nameJp: '仙台', icon: '🐮', color: 'lime', unlockCondition: 'study_required', description: '규탄과 숲의 도시', scriptFile: 'js/travel/sendai.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    sapporo: { id: 'sapporo', name: '삿포로', nameJp: '札幌', icon: '⛷️', color: 'cyan', unlockCondition: 'study_required', description: '설경과 자연', scriptFile: 'js/travel/sapporo.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    kanazawa: { id: 'kanazawa', name: '가나자와', nameJp: '金沢', icon: '🍂', color: 'amber', unlockCondition: 'study_required', description: '작은 교토', scriptFile: 'js/travel/kanazawa.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    tokyo: { id: 'tokyo', name: '도쿄', nameJp: '東京', icon: '🗼', color: 'blue', unlockCondition: 'study_required', description: '일본의 수도', scriptFile: 'js/travel/tokyo.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    okinawa: { id: 'okinawa', name: '오키나와', nameJp: '沖縄', icon: '🏖️', color: 'teal', unlockCondition: 'study_required', description: '아열대 해변 리조트', scriptFile: 'js/travel/okinawa.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    hiroshima: { id: 'hiroshima', name: '히로시마', nameJp: '広島', icon: '⛩️', color: 'orange', unlockCondition: 'study_required', description: '평화와 미야지마', scriptFile: 'js/travel/hiroshima.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    nagasaki: { id: 'nagasaki', name: '나가사키', nameJp: '長崎', icon: '⛪', color: 'rose', unlockCondition: 'study_required', description: '짬뽕과 야경', scriptFile: 'js/travel/nagasaki.js', mission: '📚 공부하세요! 그러면 열립니다.' },
    nikko: { id: 'nikko', name: '닛코', nameJp: '日光', icon: '🙈', color: 'emerald', unlockCondition: 'study_required', description: '세계유산과 자연', scriptFile: 'js/travel/nikko.js', mission: '📚 공부하세요! 그러면 열립니다.' }
};
...
function renderRegionSelection() {
    ...
    let html = `
        <!-- Shopping & Utilities (Premium Row) -->
        <div class="col-span-2 grid grid-cols-2 gap-4 mb-4">
             <button onclick="openShopping('donki')" 
                class="relative bg-gradient-to-br from-[#ffd700] to-[#ffcc00] text-slate-900 p-4 rounded-[28px] font-black text-sm shadow-xl shadow-yellow-100/50 hover:scale-[1.03] active:scale-95 transition-all flex items-center justify-center gap-3 border-b-4 border-yellow-600 group overflow-hidden">
                <div class="absolute -right-2 -bottom-2 opacity-10 text-5xl group-hover:rotate-12 transition-transform">🐧</div>
                <span class="text-xl">🏪</span> 
                <div class="text-left">
                    <div class="leading-none">돈키호테</div>
                    <div class="text-[9px] opacity-70 mt-1 uppercase tracking-tighter">Shopping Guide</div>
                </div>
             </button>
             <button onclick="openShopping('drugstore')" 
                class="relative bg-gradient-to-br from-white to-blue-50 text-blue-600 p-4 rounded-[28px] font-black text-sm shadow-xl shadow-blue-100/50 hover:scale-[1.03] active:scale-95 transition-all flex items-center justify-center gap-3 border-b-4 border-blue-200 group overflow-hidden">
                <div class="absolute -right-2 -bottom-2 opacity-10 text-5xl group-hover:rotate-12 transition-transform text-blue-500">💊</div>
                <span class="text-xl">🏥</span> 
                <div class="text-left">
                    <div class="leading-none">드럭스토어</div>
                    <div class="text-[9px] opacity-70 mt-1 uppercase tracking-tighter">Essentials</div>
                </div>
             </button>
        </div>

        <!-- Transportation Guide (Premium Full Width) -->
        <div class="col-span-2 mb-8">
            <button onclick="initTransportationGuide()" 
                class="w-full bg-slate-900 text-white p-5 rounded-[32px] shadow-2xl shadow-slate-300 hover:shadow-slate-400 transform hover:-translate-y-1 transition-all duration-300 flex items-center gap-4 group relative overflow-hidden">
                <div class="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl group-hover:bg-white/10 transition-colors"></div>
                <div class="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center group-hover:rotate-12 transition-transform border border-white/10">
                    <i class="fas fa-train text-lg text-emerald-400"></i>
                </div>
                <div class="text-left flex-1">
                    <div class="font-black text-base italic tracking-tight mb-0.5">일본 교통 완전 정복 가이드</div>
                    <div class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Passes, Routes & Travel Etiquette</div>
                </div>
                <div class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:translate-x-1 transition-transform">
        ...
        <div class="col-span-2 flex items-end gap-3 mb-6 px-2">
            <div class="h-8 w-2 bg-red-500 rounded-full"></div>
            <div>
                <h3 class="text-xl font-black text-slate-800 tracking-tighter leading-none">Travel Regions</h3>
                <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">Select your next destination</p>
            </div>
        </div>
    `;
    ...
    Object.values(japanRegions).forEach(region => {
        ...
        const lockIcon = isUnlocked ? '' : '<div class="absolute inset-0 z-20 bg-slate-900/40 backdrop-blur-[2px] flex items-center justify-center rounded-[32px] group-hover:backdrop-blur-none transition-all"><i class="fas fa-lock text-white text-3xl drop-shadow-lg scale-90 group-hover:scale-100 transition-transform"></i></div>';
        const opacity = isUnlocked ? '' : 'grayscale-[0.5]';
        const cursor = isUnlocked ? 'cursor-pointer hover:scale-[1.02] hover:-translate-y-1' : 'cursor-pointer hover:scale-[1.02]';
        const onClick = `onclick="handleRegionClick('${region.id}')"`;

        html += `
            <div id="region-card-${region.id}" ${onClick} 
                 class="group relative bg-white p-7 rounded-[40px] shadow-xl shadow-slate-200/60 border border-slate-100 flex flex-col items-center transition-all duration-300 ${cursor} ${opacity} overflow-hidden">
                
                ${lockIcon}
                
                <!-- Background Decoration -->
                <div class="absolute -top-10 -left-10 w-24 h-24 bg-${region.color}-500/5 rounded-full group-hover:scale-150 transition-transform duration-700"></div>
                
                <div class="text-6xl mb-5 transform transition-transform group-hover:scale-110 z-10">${region.icon}</div>
                <h3 class="text-lg font-black text-slate-800 mb-1 tracking-tight z-10">${region.name}</h3>
                <div class="flex items-center gap-1.5 mb-3 z-10">
                    <span class="text-[10px] font-black text-${region.color}-600 bg-${region.color}-50 px-2 py-0.5 rounded-full uppercase tracking-tighter">${region.nameJp}</span>
                </div>
                <p class="text-[11px] text-slate-400 font-bold text-center leading-snug line-clamp-2 px-1 z-10">${region.description}</p>
                
                ${!isUnlocked ? `
                    <div class="mt-4 pt-4 border-t border-slate-50 w-full text-center z-10">
                        <p class="text-[9px] font-black text-red-500 uppercase tracking-widest animate-pulse italic">Locked Content</p>
                    </div>
                ` : `
                    <div class="mt-4 pt-4 border-t border-slate-50 w-full text-center z-10 group-hover:border-${region.color}-100 transition-colors">
                        <p class="text-[9px] font-black text-slate-300 group-hover:text-${region.color}-600 uppercase tracking-widest transition-colors font-mono italic">Discover now</p>
                    </div>
                `}

                <div class="absolute inset-x-0 bottom-0 h-1.5 bg-gradient-to-r from-${region.color}-400 to-${region.color}-600 opacity-0 group-hover:opacity-100 transition-opacity"></div>
            </div>
        `;
    });
...
function updateTravelHeader(title) {
    ...
    if (headerTitle) {
        if (title.includes('지역 선택')) {
            headerTitle.innerHTML = '<span class="text-red-600">✈️</span> 일본 여행 <span class="text-slate-200 font-light">/</span> <span class="text-slate-400 text-base font-medium">Explore</span>';
        } else {
            headerTitle.innerHTML = `<span class="text-red-600">📍</span> ${title} <span class="text-slate-400 text-base font-medium ml-2">City Guide</span>`;
        }
    }
}
...
function checkRegionUnlock(region) {
    ...
    // ★★★ study_required: 아빠 외에는 모두 잠금 ★★★
    if (region.unlockCondition === 'study_required') {
        return false; // 아빠 외에는 항상 잠금
    }
...
function selectRegion(regionId, isPreview = false) {
    // [MODIFIED] Fukuoka Region Integrated - No Redirect
    // if (regionId === 'fukuoka') { ... } // Removed Redirection
    ...
    const initializeRegion = () => {
        ...
        // Fukuoka Config Injection (SPA Mode)
        if (regionId === 'fukuoka') {
            window.fukuokaConfig = { layout: 'grid', theme: 'light' }; // Force light theme for consistency
        }

        const backBtnHtml = `
            <div id="travel-back-btn" class="mb-6 flex items-center justify-between animate-fade-in-up">
                <button onclick="backToRegionSelection()" 
                    class="group flex items-center gap-3 text-slate-500 hover:text-slate-800 font-black transition-all bg-white border border-slate-100 pl-3 pr-6 py-2.5 rounded-2xl shadow-xl shadow-slate-200/50 active:scale-95">
                    <div class="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center group-hover:bg-red-50 group-hover:text-red-500 transition-colors">
                        <i class="fas fa-arrow-left text-xs"></i>
                    </div>
                    <span class="text-xs uppercase tracking-widest font-mono">Back to Maps</span>
                </button>
                
                <div class="flex items-center gap-2 px-4 py-2 bg-slate-100/50 rounded-2xl border border-slate-200/30 backdrop-blur-md">
                    <span class="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span>
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Live Itinerary</span>
                </div>
            </div>
            
            <!-- Injected Layout for City Scripts (Premium) -->
            <div class="sticky top-0 z-40 pt-2 pb-6 mb-8 transition-all">
                <div id="day-tabs"></div>
            </div>
            
            <div id="map" class="w-full h-72 rounded-[40px] mb-8 shadow-2xl shadow-slate-300/50 bg-slate-100 overflow-hidden relative border-4 border-white group/map transition-transform duration-500">
                <div class="absolute inset-0 z-10 pointer-events-none border border-slate-900/5 rounded-[36px]"></div>
            </div>
            
            <div id="itinerary-content" class="pb-24"></div>
        `;
    ...
    if (typeof window[initFuncName] === 'function') {
        ...
        if (regionId === 'fukuoka' || regionId === 'osaka') {
            loadTravelDependencies(regionId)
                .then(() => loadRegionScript(region))
                .then(initializeRegion);
        } else {
...
// Helper for Travel Dependencies (Consolidated)
function loadTravelDependencies(regionId) {
    ...
    const commonScripts = [
        ...
        'js/travel/TravelModule.js',
        'js/travel/leaflet_map.js'
    ];

    const poiDataScript = `js/travel/${regionId}_poi_data.js`;

    return new Promise(async (resolve) => {
        // Load Common Scripts First
        for (const src of commonScripts) {
        ...
        // Load Region Specific POI Data
        if (!document.querySelector(`script[src="${poiDataScript}"]`)) {
            await new Promise((r) => {
                const s = document.createElement('script');
                s.src = poiDataScript;
                s.onload = r;
